...
// ---------------------------------------------------------------------------
// Requisitions: staff-filed spending requests with supporting docs. Amounts
// at/above the configured threshold sit pending until an admin approves
// them; below it, they're booked as an expense immediately.
// ---------------------------------------------------------------------------
...
const DEFAULT_REQUISITION_THRESHOLD_CENTS = 5_000_000; // ₱50,000

export async function getRequisitionSettings(): Promise<RequisitionSettings> {
  return readJson<RequisitionSettings>("requisition-settings.json", {
    thresholdCents: DEFAULT_REQUISITION_THRESHOLD_CENTS,
  });
}

export async function saveRequisitionThreshold(thresholdCents: number, actor: string): Promise<void> {
  await writeJson("requisition-settings.json", { thresholdCents });
  await appendAudit({
    actor,
    action: "requisition_settings.updated",
    entityType: "settings",
    entityId: "requisition-settings",
    summary: `Set requisition approval threshold to ₱${(thresholdCents / 100).toLocaleString()}`,
  });
}
...
export async function listRequisitions(): Promise<LocalRequisition[]> {
  return readJson<LocalRequisition[]>("requisitions.json", []);
}
...
export async function createRequisition(input: {
  ...
  vendor?: string | null;
  amountCents: number;
  paidFrom: LocalRequisition["paidFrom"];
  supportingDocId: string | null;
}): Promise<LocalRequisition> {
  const { thresholdCents } = await getRequisitionSettings();
  const rows = await listRequisitions();
  const autoApproved = input.amountCents < thresholdCents;
  const row: LocalRequisition = {
    id: newId("requisition"),
    ...
    amountCents: input.amountCents,
    paidFrom: input.paidFrom,
    supportingDocId: input.supportingDocId,
  ...
  rows.push(row);
  await writeJson("requisitions.json", rows);
  await appendAudit({
    ...
    summary: autoApproved
      ? `Filed and auto-approved ₱${(row.amountCents / 100).toLocaleString()} requisition: ${row.description}`
      : `Filed ₱${(row.amountCents / 100).toLocaleString()} requisition for approval: ${row.description}`,
  });
  return row;
}

export async function resolveRequisition(input: {
  id: string;
  status: "approved" | "rejected";
  resolvedBy: string;
  rejectionReason?: string;
}): Promise<LocalRequisition | null> {
  const rows = await listRequisitions();
  ...
  row.rejectionReason = input.status === "rejected" ? (input.rejectionReason ?? "No reason given") : null;
  await writeJson("requisitions.json", rows);
  await appendAudit({
    actor: input.resolvedBy,
    action: input.status === "approved" ? "requisition.approved" : "requisition.rejected",
    entityType: "requisition",
    entityId: row.id,
    summary:
      input.status === "approved"
        ? `Approved ₱${(row.amountCents / 100).toLocaleString()} requisition: ${row.description}`
        : `Rejected requisition: ${row.description} (${row.rejectionReason})`,
  });
  return row;
}

// ---------------------------------------------------------------------------
// Sales agents, client-to-agent tagging (for commission attribution), and
// commission payouts. Tagging is a standalone override keyed by client id —
// the same shape as ContactOverride above — so it works whether the client
// itself lives in Supabase or in the local store, and for masterlist
// clients that predate agent tracking entirely.
// ---------------------------------------------------------------------------
...
export async function listAgents(): Promise<LocalAgent[]> {
  return readJson<LocalAgent[]>("agents.json", []);
}
...
export async function createAgent(input: {
  ...
  await appendAudit({
    ...
    entityType: "agent",
    entityId: agent.id,
    summary: `Added sales agent ${agent.name} (${agent.commissionRatePercent}% commission)`,
  });
  return agent;
}

export interface ClientAgentTag {
  clientId: string;
  clientName: string;
  agentId: string;
  agentName: string;
...
export async function getClientAgentTags(): Promise<Record<string, ClientAgentTag>> {
  return readJson<Record<string, ClientAgentTag>>("client-agent-tags.json", {});
}

export async function setClientAgentTag(input: {
  clientId: string;
  clientName: string;
  agentId: string;
  agentName: string;
  ...
  const tags = await getClientAgentTags();
  tags[input.clientId] = {
    clientId: input.clientId,
    clientName: input.clientName,
  ...
  await writeJson("client-agent-tags.json", tags);
  await appendAudit({
    actor: input.taggedBy,
...
export interface LocalCommissionPayout {
  id: string;
  agentId: string;
  amountCents: number;
  paidAt: string;
...
export async function listCommissionPayouts(): Promise<LocalCommissionPayout[]> {
  return readJson<LocalCommissionPayout[]>("commission-payouts.json", []);
}
...
export async function recordCommissionPayout(input: {
  ...
  payouts.push(payout);
  await writeJson("commission-payouts.json", payouts);
  await appendAudit({
    actor: input.recordedBy,
    action: "commission.paid_out",
    entityType: "agent",
    entityId: input.agentId,
    summary: `Paid out ₱${(payout.amountCents / 100).toLocaleString()} commission to ${input.agentName}`,
  });
  return payout;
}
