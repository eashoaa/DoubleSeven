...
export function Sidebar({ role }: { role: Role }) {
  ...
  useGSAP(
    () => {
      ...
      // The logo's own wrapper gets an explicit pixel target (not "auto")
      // and shares the exact same duration as the container width tween,
      // so it can't ever finish first and read as an instant pop while the
      // rest of the sidebar is still easing in.
      tl.to(".logo-clip", { width: collapsed ? 0 : expandedWidth - 24 }, 0);
      tl.to(".logo-fade", { opacity: collapsed ? 0 : 1 }, 0);
